 ...
 * @since       1.6
 */
class NtripModelUser_Man_Album extends JModelAdmin
{
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ntrip.album', 'album', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ntrip.edit.album.data', array());
		...
		if (empty($data))
		{
			...
			if ($this->getState('album.id') == 0)
			{
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_ntrip.albums.filter.category_id')));
			}
		}
	...
	function getItem($pk = null)
	{
	    ...
	    $item->other_images = NtripHelper::getImages($item->id, 'albums');
