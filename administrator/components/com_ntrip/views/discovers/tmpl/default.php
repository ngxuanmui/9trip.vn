...
?>
<form action="<?php echo JRoute::_('index.php?option=com_ntrip&view=discovers'); ?>" method="post" name="adminForm" id="adminForm">
			...
			<tr>
				...
				<th width="10%">
					...
					<?php if ($canOrder && $saveOrder): ?>
						<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'discovers.saveorder'); ?>
					<?php endif;?>
				</th>
				<th width="5%">
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<?php if ($item->checked_out) : ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'discovers.', $canCheckin); ?>
					<?php endif; ?>
					<?php if ($canEdit) : ?>
						<a href="<?php echo JRoute::_('index.php?option=com_ntrip&task=discover.edit&id='.(int) $item->id); ?>">
				...
				<td class="order">
					<?php if ($canChange) : ?>
						<?php if ($saveOrder) : ?>
							<?php if ($listDirn == 'asc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, (@$this->items[$i-1]->catid == $item->catid), 'discovers.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, (@$this->items[$i+1]->catid == $item->catid), 'discovers.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php elseif ($listDirn == 'desc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, (@$this->items[$i-1]->catid == $item->catid), 'discovers.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, (@$this->items[$i+1]->catid == $item->catid), 'discovers.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php endif; ?>
						<?php endif; ?>
				...
				<td class="center">
					<a href="<?php echo JRoute::_('index.php?option=com_ntrip&view=comments&type=discover&item_id='.$item->id); ?>">
						<?php echo JText::_('COM_NTRIP_HEADING_COMMENT'); ?>
					</a>
				</td>
				<td class="center">
