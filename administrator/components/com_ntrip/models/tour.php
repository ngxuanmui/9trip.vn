 ...
 * @since       1.6
 */
class NtripModelTour extends JModelAdmin
{
	/**
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'Tour', $prefix = 'NtripTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ntrip.tour', 'tour', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ntrip.edit.tour.data', array());
		...
		if (empty($data))
		{
			...
			if ($this->getState('tour.id') == 0)
			{
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_ntrip.tours.filter.category_id')));
			}
		}
	...
	function getItem($pk = null)
	{
	    ...
	    $item->other_images = NtripHelper::getImages($item->id, 'tours');
...
//			NtripHelper::updateImages($id, $currentImages, 'tours');
			...
			// Upload thumb
			$item = $this->getItem();
			$data['images'] = NtripHelper::uploadImages('images', $item, $delImage, 'tours');
			...
			// update content
			$content = NtripHelper::copyFilesOnSave($data['description'], 'tours', $id);
