 ...
 * @since       1.6
 */
class NtripModelService extends JModelAdmin
{
	/**
	 * @var    string  The prefix to use with controller messages.
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_NTRIP_SERVICE';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'Service', $prefix = 'NtripTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ntrip.service', 'service', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ntrip.edit.service.data', array());
		...
		if (empty($data))
		{
			...
			if ($this->getState('service.id') == 0)
			{
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_ntrip.services.filter.category_id')));
			}
		}
	...
	function getItem($pk = null)
	{
	    ...
	    $item->other_images = NtripHelper::getImages($item->id, 'services');
...
//			NtripHelper::updateImages($id, $currentImages, 'services');
			...
			NtripHelper::updateImages($id, $currentImages, $currentDesc, 'services');
			...
			if (isset($_POST['tmp_other_img']))
			{
				...
				// Insert images
				NtripHelper::insertImages($id, $_POST['tmp_other_img'], $_POST['tmp_desc'], 'services');
			}
			...
			// update content
			$content = NtripHelper::copyFilesOnSave($data['description'], 'services', $id);
